...
import { getUserById, supportRequestsForSite } from "@/lib/db";
...
export default async function SupportPage() {
  ...
  const rawRequests = await supportRequestsForSite(site.id);
  const requests = await Promise.all(
    rawRequests.map(async (r) => ({
      ...r,
      requesterName: (await getUserById(r.userId))?.name ?? "You",
    }))
  );
